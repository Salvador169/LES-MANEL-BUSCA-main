...
from .tables import ContratosTable, FaturasTable, ReclamacaoTable
from django_filters.views import FilterView
from .filters import ContratosFilter, FaturasFilter, ReclamacaoFilter
...
def contrato_update_view(request, id):
    ...
    form = ContratoForm(request.POST or None, current_user=Cliente.objects.get(id=request.user.id), instance=obj)
    if form.is_valid():
        form.save(id)
        return redirect(reverse('PaymentManagement:contrato-list'))
    context ={
        'form': form,
    }
    return render(request, "create.html", context)
...
def contrato_validate_aprove_view(request, id):
    ...
    contrato.save()
    return redirect('PaymentManagement:contrato-list')
...
def fatura_detail_view(request, id):
    ...
    if fatura.pagamentoid.contratoid is not None and fatura.pagamentoid.estado_do_pagamento=="Pago":
        ...
        recibo=False
        fatura=fatura
        registo = RegistoMovimento.objects.filter(matricula=fatura.pagamentoid.contratoid.matricula)
    else:
        faturarecibo=fatura
        recibo=False
        fatura=False
...
def payment_prove_create_view(request, id):
    if request.method == "POST":
        ...
        if request_file:
                ...
                pagamento.estado_do_pagamento = "Pago"
                pagamento.save()
                return redirect('homepage')
...
def payment_create_view(request, id, payid):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            fatura.nifEmpresa = 123456789
            fatura.nomeEmpresa = "Parques Lda."
            fatura.moradaEmpresa = "Avenida da Liberdade, Lisboa"
...
def payment_create_view_reference(request, id, payid):
    ...
    if pagamento.referencia == None:
        pagamento.referencia = reference
    else:
...
class ReclamacaoListView(SingleTableMixin, FilterView):
    table_class = ReclamacaoTable
    model = Reclamacao
    template_name = 'reclamacao_list.html'
